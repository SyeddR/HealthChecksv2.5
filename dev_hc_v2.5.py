...
def main(argv):
   ...
   try:
      ...
      print './dev_hc.v2.3.py -f <PMs IP file> -i < IS IP file> -l <Link_Status: Yes or No> -s <email subject>'
      sys.exit()
   for opt, arg in opts:
      if opt == '-f':
         pmip = arg
   ...
   wdir=os.path.dirname(os.path.realpath(__file__)) ## to get the current directory where the script exist
   ...
   pm_out= open("%s/%s_PM_Healthcheck.csv"%(wdir,Deployment),"wb") 
   ...
   is_out= open("%s/%s_IS_Healthcheck.csv"%(wdir,Deployment), "wb") 
   ...
   'Tcm connection shortages': ['if_\d:\d*',IS_col.index('TCM Conn shortages')],
   ...
   if ln_status == 'Yes' or ln_status =='yes':
     write_link_summary(link_data,is_output,wdir,Voice_IS,is_ip,IS_col) ###(unsorted is_output)
   print  "Probes_Interface_link_status have been created"
   print PM_err_summ
   print IS_err_summ
   
   ####Wrting error Summary
   write_error("%s/%s_Error_Summary.html"%(wdir,Deployment),PM_err_summ,IS_err_summ,is_list_output,IS_err,pm_list_output,PM_err)
   ...
   print html
   send_email(To, From,wdir,ln_status,SMTP_IP,html)
 ...
 if int(ip_status[0]) > 0 and hostname:
    ...
    if re.search('GPM',d):
     comm= [ """ hostname """,
            ...
            """ cat /opt/NetScout/rtm/pa/bin/decoderelease.properties|grep -o "[1-5].*" """,
            """ ntpq -pn |grep u|awk '{print $1}' |tr '\\n' ';' """,
            """ ntpq -pn |grep u|awk '{print $3}' |tr '\\n' ';' """,
            """ ntpq -pn |grep u|awk '{print $8}' |tr '\\n' ';' """,
            ...
            """ racadm getversion |grep -i iDRAC| cut -d= -f2 """,
            """ racadm getniccfg | grep "IP Address"|head -n 1|cut -d= -f2 """,
            """ racadm getniccfg | grep "Link Detected"|cut -d= -f2 """,
            """ df | grep opt|awk '{print $2/1073741824}' """,
            """ df -h |grep opt|grep -o "[0-9]*%" """,
            """ df -h |grep "/$"|grep -o "[0-9]*%" """,
            """ cat /opt/NetScout/rtm/log/memorylog-globalm-$(date --date="today" |awk '{print $1}'|tr '[:upper:]' '[:lower:]').txt|tail -1 |cut -d, -f4|  grep -o "(.*)"|grep -o "[0-9]*%" """,
            """ echo "N/A" """,
            """ echo "N/A" """,
            """ echo "N/A" """,
            """ echo "N/A" """,
            ...
            """ echo "N/A" """,
            """ echo "N/A" """,
            """ cat /opt/NetScout/rtm/log/nssituationmemorylog-$(date --date="today" |awk '{print $1}'|tr '[:upper:]' '[:lower:]').txt|tail -1 |cut -d, -f4| grep -o "(.*)"|grep -o "[0-9]*%" """,
            """ echo "N/A" """,
            """ echo "N/A" """,
            """ echo "N/A" """,
            """ /opt/NetScout/rtm/bin/PS|grep postgres """,
            """ /opt/NetScout/rtm/bin/PS|grep http """,
            """ ps -ef | grep pgsql_stealth_db | wc -l """,
            """ cd /opt/NetScout/rtm/log;find *$( date --date="today" |awk '{print $1}'|tr '[:upper:]' '[:lower:]').txt -mtime -1|xargs grep -i "debug ini"|sed -n  's/\\(.*\\)debuglog.*\\(\\[.*\\].*Debug\\).*/ \\1 \\2/p'|grep -o ".*]" """,
            """ cd /opt/NetScout/rtm/log;find *$( date --date="yesterday" |awk '{print $1}'|tr '[:upper:]' '[:lower:]').txt -mtime -1|xargs grep -i "debug ini"|sed -n  's/\\(.*\\)debuglog.*\\(\\[.*\\].*Debug\\).*/ \\1 \\2/p'|grep -o ".*]" """,
            """ /opt/NetScout/rtm/bin/PS|grep ngenius |awk '{print $7 }'|tr '\\n' ' '""",
            """ echo " " """,
            """ cd /opt/NetScout/rtm/log;grep -i "isBlackOut = true"  debuglog-$( date --date="today" |awk '{print $1}'|tr '[:upper:]' '[:lower:]').txt |grep WebxRemoteRequestDispatcherService | sed -n 's/.*Server :\\( .*\\)RMI.*/\\1/p'|uniq """,
            """ cd %s; ls -Art | tail -1|xargs stat| grep Change |sed -n 's/Change:\\(.*-[0-9][0-9]-[0-9][0-9]\\).*/\\1/p' """%Config_Backup,
            """ grep "Sense code: .*" /var/log/messages |wc -l""",
            """ grep "Request to start paservice received" /opt/NetScout/rtm/log/paservice_$( date --date="today" |awk '{print $1}'|tr '[:upper:]' '[:lower:]').log"""]
      ...
      comm= [ """ hostname """,
            ...
            """ sar | awk '{print $9}'|grep "[0-9].*" |sort|head -1| awk '{x=100-$0};END{print x}' """,
            """ omreport system summary | grep -i version| head -1|cut -d: -f2 """,
            """ racadm getversion |grep iDRAC| cut -d= -f2 """,
            """ racadm getniccfg | grep "IP Address"|head -n 1|cut -d= -f2 """,
            """ racadm getniccfg | grep "Link Detected"|cut -d= -f2 """,
            """ df | grep opt|awk '{print $2/1073741824}' """,
            """ df -h |grep opt|grep -o "[0-9]*%" """,
            """ df -h |grep "/$"|grep -o "[0-9]*%" """,
            """ cat /opt/NetScout/rtm/log/memorylog-$(date --date="today" |awk '{print $1}'|tr '[:upper:]' '[:lower:]').txt|tail -1 |cut -d, -f4|  grep -o "(.*)"|grep -o "[0-9]*%" """,
            """ cat /opt/NetScout/rtm/log/asi2xwarehousememorylog-$(date --date="today" |awk '{print $1}'|tr '[:upper:]' '[:lower:]').txt|tail -1 | cut -d, -f4| grep -o "(.*)"|grep -o "[0-9]*%" """,
            """ cat /opt/NetScout/rtm/log/asi2xservicewarehousememorylog-$(date --date="today" |awk '{print $1}'|tr '[:upper:]' '[:lower:]').txt|tail -1 |cut -d, -f4|  grep -o "(.*)"|grep -o "[0-9]*%" """,
            """ cat /opt/NetScout/rtm/log/webxpresentmemorylog-$(date --date="today" |awk '{print $1}'|tr '[:upper:]' '[:lower:]').txt|tail -1 |cut -d, -f4|  grep -o "(.*)"|grep -o "[0-9]*%" """,
            """ cat /opt/NetScout/rtm/log/uccontentmemorylog-$(date --date="today" |awk '{print $1}'|tr '[:upper:]' '[:lower:]').txt|tail -1 | cut -d, -f4| grep -o "(.*)"|grep -o "[0-9]*%" """,
            """ cat /opt/NetScout/rtm/log/loggermemorylog-$(date --date="today" |awk '{print $1}'|tr '[:upper:]' '[:lower:]').txt|tail -1 |cut -d, -f4|  grep -o "(.*)"|grep -o "[0-9]*%" """,
            """ cat /opt/NetScout/rtm/log/flowloggermemorylog-$(date --date="today" |awk '{print $1}'|tr '[:upper:]' '[:lower:]').txt|tail -1 | cut -d, -f4| grep -o "(.*)"|grep -o "[0-9]*%" """,
            """ cat /opt/NetScout/rtm/log/fdsindexingmemorylog-$(date --date="today" |awk '{print $1}'|tr '[:upper:]' '[:lower:]').txt|tail -1 | cut -d, -f4| grep -o "(.*)"|grep -o "[0-9]*%" """,
            """ cat /opt/NetScout/rtm/log/asi2xloggermemorylog-$(date --date="today" |awk '{print $1}'|tr '[:upper:]' '[:lower:]').txt|tail -1 | cut -d, -f4| grep -o "(.*)"|grep -o "[0-9]*%" """,
            """ cat /opt/NetScout/rtm/log/analyticsmemorylog-$(date --date="today" |awk '{print $1}'|tr '[:upper:]' '[:lower:]').txt|tail -1 | cut -d, -f4| grep -o "(.*)"|grep -o "[0-9]*%" """,
            """ cat /opt/NetScout/rtm/log/whmemorylog-$(date --date="today" |awk '{print $1}'|tr '[:upper:]' '[:lower:]').txt|tail -1 | cut -d, -f4| grep -o "(.*)"|grep -o "[0-9]*%" """,
            """ cat /opt/NetScout/rtm/log/webxreportgenmemorylog-$(date --date="today" |awk '{print $1}'|tr '[:upper:]' '[:lower:]').txt|tail -1 |cut -d, -f4|  grep -o "(.*)"|grep -o "[0-9]*%" """,
            """ cat /opt/NetScout/rtm/log/flowrollupmemorylog-$(date --date="today" |awk '{print $1}'|tr '[:upper:]' '[:lower:]').txt|tail -1 |cut -d, -f4| grep -o "(.*)"|grep -o "[0-9]*%" """,
            """ cat /opt/NetScout/rtm/log/nssituationmemorylog-$(date --date="today" |awk '{print $1}'|tr '[:upper:]' '[:lower:]').txt|tail -1 |cut -d, -f4| grep -o "(.*)"|grep -o "[0-9]*%" """,
            """ cat /opt/NetScout/rtm/log/flowloggerdebuglog-$(date --date="yesterday" |awk '{print $1}'|tr '[:upper:]' '[:lower:]').txt|grep total.*flows |cut -d= -f2|awk 'ORS=NR%4?" ":"\\n"'|awk '{print $1+$2+$3+$4}'|sort -n|tail -1 """,
            """ cat /opt/NetScout/rtm/log/asi2xloggerdebuglog-$(date --date="yesterday" |awk '{print $1}'|tr '[:upper:]' '[:lower:]').txt| grep DBONEStatsCollector:WRITE:AGGREGATED|grep -o "writtenRows=[0-9]*"|cut -d= -f2|sort -n |tail -1 """ ,
            """ cat /opt/NetScout/rtm/log/asi2xloggereventlog-$(date --date="yesterday" |awk '{print $1}'|tr '[:upper:]' '[:lower:]').txt|grep -o "totalPollCycleTime=[0-9]*"|cut -d= -f2|sort -n |tail -1""",
            """ /opt/NetScout/rtm/bin/PS|grep postgres """,
            """ /opt/NetScout/rtm/bin/PS|grep http """,
            """  ps -ef | grep pgsql_stealth_db | wc -l """,
            """ cd /opt/NetScout/rtm/log;find *$( date --date="today" |awk '{print $1}'|tr '[:upper:]' '[:lower:]').txt -mtime -1|xargs grep -i "debug ini"|sed -n  's/\\(.*\\)debuglog.*\\(\\[.*\\].*Debug\\).*/ \\1 \\2/p'|grep -o ".*]" """,
            """ cd /opt/NetScout/rtm/log;find *$( date --date="yesterday" |awk '{print $1}'|tr '[:upper:]' '[:lower:]').txt -mtime -1|xargs grep -i "debug ini"|sed -n  's/\\(.*\\)debuglog.*\\(\\[.*\\].*Debug\\).*/ \\1 \\2/p'|grep -o ".*]" """,
            """ /opt/NetScout/rtm/bin/PS|grep ngenius |awk '{print $7 }'|tr '\\n' ' '""",
            """ find /opt/NetScout/rtm/log -mtime -2 -name "asi2xloggerxerrorlog-*"|xargs grep "timed out" |sed -n 's/.*\\/\\([1-9].*\\):.*/\\1/p'|sort|uniq """,
            """ echo " " """,
            """ cd %s; ls -Art | tail -1|xargs stat| grep Change |sed -n 's/Change:\\(.*-[0-9][0-9]-[0-9][0-9]\\).*/\\1/p' """%Config_Backup,
            """ grep "Sense code:.*" /var/log/messages |wc -l""" ,
            """ grep "Request to start paservice received" /opt/NetScout/rtm/log/paservice_$( date --date="today" |awk '{print $1}'|tr '[:upper:]' '[:lower:]').log"""           ]
    ...
    for i in comm:
        ...
        count+=1
    if ip_version == 'IPv6':
       p= subprocess.Popen(["ssh" ,"-6",Login+'@'+j,command], stdout=subprocess.PIPE)
       output,err= p.communicate()
    else :
       p= subprocess.Popen(["ssh",Login+'@'+j,command], stdout=subprocess.PIPE)
       output,err= p.communicate()
    print output
...
def IS_ssh (j,link_data):
 ...
 if int(ip_status[0]) > 0 and hostname:
    ...
    comm= [ """ hostname """,
     ...
     """ date """,
     """ hwclock """,
     """ ipmitool lan print |grep -i "IP Address"| tail -1| cut -d: -f2 """,
     """ pkill -9 localconsole;echo -e "11\\nget agent\\nexit\\n"| /opt/NetScout/rtm/bin/localconsole| grep model_number|awk '{print $3}' """,
     """  echo -e "11\\nget agent\\nexit\\n"| /opt/NetScout/rtm/bin/localconsole| grep software_version| awk '{print $2,$3,$4,$5}' """,
     """ echo -e "11\\n get asi_mode \\nexit\\n"| /opt/NetScout/rtm/bin/localconsole|grep asi_mode|sed -n 's/.*set.*to*\\(.*\\)/\\1/p' """,
     """  echo -e "11\\nget agent\\nexit\\n"| /opt/NetScout/rtm/bin/localconsole| grep serial|head -1|awk '{print $2}' """, ##
     """ uname -a |cut -d' ' -f3 """,
     """ grep -i processor /proc/cpuinfo |wc -l """,
     """ cat /proc/meminfo |grep MemTotal|grep -o -e "[0-9].*[0-9]" """,
     """ uptime |awk -F, '{print $1}' """,
     """ uptime |grep -o load.*|cut -d: -f2 """,
     """ cd /opt/platform/nshwmon/log/nshwmon-logfiles/;ls -Art nshwmon* | tail -n 2 | head -n 1 |xargs cat|grep "Data.*Array"|sort|uniq|wc -l """,
     """ cd  /opt/platform/nshwmon/log/nshwmon-logfiles/;ls -Art nshwmon* | tail -n 2 | head -n 1 |xargs cat|grep Disk| sort|uniq|wc -l """,
     """ cd  /opt/platform/nshwmon/log/nshwmon-logfiles/;ls -Art nshwmon* | tail -n 2 | head -n 1 |xargs cat|grep Disk|grep -e "FAILED" -e "AVAILABLE" -e "MISSING" |sort| uniq |wc -l """,
     """ cd  /opt/platform/nshwmon/log/nshwmon-logfiles/;ls -Art nshwmon* | tail -n 2 | head -n 1 |xargs cat|grep Disk|grep -e "FAILED" -e "AVAILABLE" -e "MISSING" |sort| uniq|grep -o -e " Infin.* Disk" -e " ESU.* Disk"|awk '{print $1,$3,$4}' |awk {print} ORS=" ," """,
     """ cd  /opt/platform/nshwmon/log/nshwmon-logfiles/;ls -Art nshwmon* | tail -n 2 | head -n 1 |xargs cat|grep Disk|grep -i "FAILING"|sort| uniq |wc -l """,    
     """ cd  /opt/platform/nshwmon/log/nshwmon-logfiles/;ls -Art nshwmon* | tail -n 2 | head -n 1 |xargs cat|grep Disk|grep -i "FOREIGN"|sort| uniq |wc -l """,
     """ cd  /opt/platform/nshwmon/log/nshwmon-logfiles/;ls -Art nshwmon* | tail -n 2 | head -n 1 |xargs cat|grep "FAN STATUS"|sort|uniq|head -n 1| awk '{print$7}' """,
     """ cd  /opt/platform/nshwmon/log/nshwmon-logfiles/;ls -Art nshwmon* | tail -n 2 | head -n 1 |xargs cat|grep "POWER SUPPLY STATUS"|sort|uniq|head -n 1| awk '{print$9}' """,
     """ cd  /opt/platform/nshwmon/log/nshwmon-logfiles/;ls -Art nshwmon* | tail -n 2 | head -n 1 |xargs cat|grep "TEMPERATURE STATUS"|sort|uniq|head -n 1| awk '{print$8}' """,
     """ cd  /opt/platform/nshwmon/log/nshwmon-logfiles/;ls -Art nshwmon* | tail -n 2 | head -n 1 |xargs cat|grep "VOLTAGE STATUS"|sort|uniq|head -n 1| awk '{print$8}' """,
     """ ntpq -pn |grep u|awk '{print $1}'|tr '\\n' ';'  """,
     """ ntpq -pn |grep u|awk '{print $3}'|tr '\\n' ';'  """,
     """ ntpq -pn |grep u|awk '{print $8}'|tr '\\n' ';'  """,
     """ find /core -mtime -2 -name "core.*" |wc -l """,
     """ ethtool eth0 |grep Duplex |cut -d: -f2 """,
     """ df -h |grep / |head -1| awk '{print $5}' """,
     """ df -h |grep metadata|grep -o "[0-9]*%" """,
     """ df -h |grep xdr|grep -o "[0-9]*%" """,
     """ df -h| grep xdr |awk '{print $2}' """,
     """ cd /xdr;stat /xdr/*/$(ls -t /xdr|head -1|xargs ls -t|tail -1)|grep ..:..:..|head -1|cut -d: -f2,3;cd /opt""",
     """  echo `/opt/NetScout/rtm/tools/printva |grep DataSize|cut -d: -f3|sed -n 's/\\(.*\\)GB.*/\\1/p'`""",
     """  pkill -9 localconsole;echo -e  "exit\\n" | /opt/NetScout/rtm/bin/localconsole|grep "Server Add" |sed -n 's/.*Address\\(.*\\)/\\1/p' """,
     """ echo -e "15\nexit\n"| /opt/NetScout/rtm/bin/localconsole|grep "Voice and Video Quality"|sed -n 's/.*Quality.*\\(o.*\\)/\\1/p' """,
     """ echo -e  "11\\nget agent\\nexit\\n" | /opt/NetScout/rtm/bin/localconsole|grep "memory size"|awk '{print $3}' """,
     """ echo -e  "11\\n get dump free_mem\\nexit\\n" | /opt/NetScout/rtm/bin/localconsole|grep Free|cut -d= -f2|grep -o .*[0-9] """,
     """ echo -e "11\\n get table_size_allocation ctrl \\nexit\\n"| /opt/NetScout/rtm/bin/localconsole| grep -A6 "Ifn.*Percentage" | grep -v -e Ifn -e "^\s*$" -e "%"|awk '{print $1,$2,$3,$4}' OFS=- ORS=";" """,
     """ echo -e "11\\n get dump sysuptime\\nexit\\n"| /opt/NetScout/rtm/bin/localconsole""",
     """ /opt/NetScout/rtm/bin/PS| sed -n 2,15p|awk  '{print $7}' |tr '\\n' ' ' """,
     """ echo -e  "11\\n set auto_scroll on\\nget dump tables\\nexit\\n" | /opt/NetScout/rtm/bin/localconsole |grep ".:.*:.*"|grep -e "`date +%m-%d-%Y`" -e `date --date="yesterday" +%m-%d-%Y`|sed  's/- ses/ses/p'|uniq |awk '{print $1,$2,$6}' OFS=: ORS=",   \n" """ ]
    ### For packet retention
    comm.append(""" /opt/NetScout/rtm/tools/printstore -pkt|head -20| sed -n 7,20p |grep  m*Time""" )
    ### For Erros in /var/log messages "USB, XDR, SATA"
    comm.append(""" grep -e "couldn't allocate port*usb_device" -e "XFS_WANT_CORRUPTED_RETURN" -e "SATA" /var/log/messages""" )
    ### Dengine greater than 8 hour
    comm.append(""" ps -eo pid,etime,comm | awk '{if($2~/-.*:.*:/ || $2~/0?[8-9]:.*:.*/ || $2~/[1-2][0-9]:.*:.*/) print $2,$3}'|grep dengine """)
    If_comm=''
    sip_comm=''
    gtp_corr_comm=''
    vifn_mode_comm=''
    ...
    if j in link_data:
       for interface in link_data[j]['IF']:
             sip= """ echo "if_%s:"|tr '\\n' ' '; echo -e "11\\n get dump mobile_tables %s\\nexit\\n"| /opt/NetScout/rtm/bin/localconsole|grep -o ses_sip_db.*|awk '{print $ 3,$5,$6,$10}' OFS=',';"""%(interface,interface)
             If= """ echo "if_%s"|tr '\\n' ':';echo -e  "11\\nset curr_interface %s\\nget interface_options %s\\nexit\\n" | /opt/NetScout/rtm/bin/localconsole|grep -i "interface type"|awk '{print $4}';"""%(interface,interface,interface)
             gtp_corr=""" echo "if_%s"|tr '\\n' ':';echo -e  "11\\nget dump scn %s \\nexit\\n" | /opt/NetScout/rtm/bin/localconsole|grep -i "TOT_NON_CORRELATED_DATA_PKTS:"|grep -o GTPv2.*|awk '{print $5}';"""%(interface,interface)
             vifn_mode= """ echo "if_%s"|tr '\\n' ':';echo -e  "11\\nset curr_interface %s\\nget interface_options %s\\nexit\\n" | /opt/NetScout/rtm/bin/localconsole|grep -i "vifn_mode"|awk '{print $3}';"""%(interface,interface,interface)
             Tcm_conn= """echo "if_%s"|tr '\\n' ':';echo -e "11\nget dump tcm %s\nexit\n"|localconsole|grep "TCM connection shortage"|awk '{print $4}';"""%(interface,interface)
             If_comm+=If
             sip_comm+=sip
...
def send_email(To, From,wdir,ln_status,SMTP_IP,html):
 global Deployment
 ...
 #msg.attach(part5)
 if ln_status == 'Yes' or ln_status == 'yes':
       fp= open("%s/%s_IS_Interface_Stats.html"%(wdir,Deployment), 'rb')
       a=fp.read().rstrip()
...
def write_link_summary(Data,is_output,wdir,Voice_IS,is_ip,IS_col): 
   global Deployment
   ###Interface Counts
   IF_count=0
   IS_unacc=[]
   ...
   html+= """ <h2 style="font-size:90%"> **** Peak Packet Drops % = (Peak Dropped packets /(Total packets in peak hr+ Peak Dropped packets))*100 </h2> """
   ...
   html+=""" <tr> <th>Probe IP</th><th>Server IP</th><th>Model</th><th>Software Version</th><th>CPUs</th><th>Total Physical Memory (GB)</th> <th>nsprobe Memory (MB)\
     ...
     <th>Peak_PPS_Time (last 24 hr)</th></th><th> Dropped Packets (peak value in last 24 hr)</th><th> Peak Packet Drops % (1 hr res) </th><th>Last hour PPS (K)</th> """
   if Voice_IS:
      ...
      #####  Total sip_db Max_session per box
         ...
         for j in Data[i]['IF']:
                 ...
                 if int(vital_stats[4]) !=0: 
                               ...
                               pkt_drops_pct= round(int(vital_stats[4])/float(vital_stats[7]+int(vital_stats[4])),4)
                 ...
                 #### Active Streams
                 if Voice_IS:
                     if re.search('[0-9]',str(vital_stats[6])):
                                    html+="""<td> %s </td>"""%vital_stats[6]  
                     else:
                                html+="""<td> null </td>"""                  
                 #### GTP correlation
                 #if gtp_corr != 'NA' or None:
                 #   gtpv2_data_corr=re.findall('if_%s:(.*)'%j,gtp_corr)[0]
                 #   if re.search('\(.*\)',gtpv2_data_corr):
   ...
   html+="</table>"           
   file=open('%s/%s_IS_Interface_Stats.html'%(wdir,Deployment),"wb")
...
def get_link_data(PM_ip,Data,is_list,Voice_IS):
      Link_Data={}
      IS_L=[]
      input_comm="""  ls -l /opt/NetScout/rtm/database/dbone/$(date +%Y-%m-%d)/vitalstats-hourly|grep -v total|awk '{print $9}'|cut -d. -f1|awk -F- '{print $3,$4}' """
      ...
      for i in IS_L:
          ...
          for j in Link_Data[i]['IF']:
             if re.search('.*\..*\..*\..*',i):
                    ...
                    ip=i.replace(":","_")
             SQL_comm=""" cd /opt/NetScout/rtm/bin;if [ -d  "link_data" ]; then cd link_data;else mkdir link_data;cd link_data;fi;"""
             SQL_comm+=""" echo "SELECT a.appid,a.targettime,a.pps,a.util,b.lh_pps FROM( SELECT appid,targettime, (vitalstats_packetsout+vitalstats_packetsin) as PPS,((vitalstats_octetsout+vitalstats_octetsin)/450000)as UTIL FROM hourly_vitalstats_%s_%s WHERE appid=184549377 ) a JOIN (SELECT appid,(vitalstats_packetsout+vitalstats_packetsin)as lh_pps FROM hourly_vitalstats_%s_%s WHERE appid=184549377 ORDER BY targettime DESC LIMIT 1 )b on a.appid=b.appid ORDER BY pps DESC LIMIT 1;" > input_pps_lu;"""%(ip,j,ip,j)
             SQL_comm+="""  echo "SELECT appid,targettime, (vitalstats_packetsout+vitalstats_packetsin)as Dp FROM hourly_vitalstats_%s_%s WHERE appid=184549384 ORDER BY Dp DESC LIMIT 1;" > input_dp;"""%(ip,j)
             SQL_comm+=""" /opt/NetScout/rtm/bin/nGeniusSQL.sh /opt/NetScout/rtm/bin/link_data/input_pps_lu /opt/NetScout/rtm/bin/link_data/out_pps_lu_%s_%s;/opt/NetScout/rtm/bin/nGeniusSQL.sh /opt/NetScout/rtm/bin/link_data/input_dp /opt/NetScout/rtm/bin/link_data/out_dp_%s_%s; """%(ip,j,ip,j)
             if Voice_IS:
                SQL_comm+="""  echo "SELECT MAX(sum)  FROM (SELECT sum(activestreamsin) FROM hourly_uc_kpi_%s_%s GROUP BY targettime) AS foo" > input_uckpi;"""%(ip,j)
                SQL_comm+=""" /opt/NetScout/rtm/bin/nGeniusSQL.sh /opt/NetScout/rtm/bin/link_data/input_uckpi /opt/NetScout/rtm/bin/link_data/out_uckpi_%s_%s """%(ip,j)
             else: 
                SQL_comm+=""" echo "None" > /opt/NetScout/rtm/bin/link_data/out_uckpi_%s_%s """%(ip,j)
             SQL_output="""  cd /opt/NetScout/rtm/bin/link_data;grep 184549377 out_pps_lu_%s_%s|awk -F, '{print $4,$3,$2,$5}' OFS=";"|tr "\n" ";";grep 184549384 out_dp_%s_%s| awk -F, '{print $3,$2}' OFS=";"|tr "\n" ";";if [ -f out_uckpi_%s_%s ]; then tail -1  out_uckpi_%s_%s;fi """%(ip,j,ip,j,ip,j,ip,j)
             ...
             if re.search('[0-9]',data_out[6]):
                data_out[6]=round(int(data_out[6])/float(3600000),2) ### Active Streams 
             print data_out
             data_index='data_'+j
             #if tot_pkts:
...
def s1_nas_deciphering(is_output,IS_col):
     ...
     if S1_ip:
         for ip in S1_ip:
           S1_output.update({ip:[]})
           for inf in S1_ip[ip]:
              command="""pkill -9 localconsole;echo if_%s|tr '\\n' ':';echo -e "11\\nget dump nas_msg %s perf\\nexit\\n"| /opt/NetScout/rtm/bin/localconsole|grep "01.*(%%)"|head -1|awk '{print $2}'|sed -n 's/\\(.*[0-9]\\).*/\\1/p' """%(inf,inf)
...
def is_output_post_processing(is_output,IS_col):
  ...
  for ip in is_output:
           ...
           ########### ns probe uptime
           try:
                is_output[ip][IS_col.index('nsprobe uptime')]= re.findall('.*>(.*)',s)[0].rstrip()
           except:
